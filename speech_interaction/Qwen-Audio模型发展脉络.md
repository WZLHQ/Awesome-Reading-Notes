...
训练过程包含预训练和后训练。你想模型具备什么样的能力，在预训练阶段模型就会获取一些基本能力；在后训练阶段，这种能力被强化，主要体现在模型的回答更好/高情商回复。

预训练和后训练，其实可以比喻成上学阶段和工作阶段。预训练和后训练具体应该训练啥，这取决于你期待模型具备啥能力。把自己比作一个模型，假设你以后想当一个算法工程师，那么预训练阶就是在大学里学习计算机相关的内容，后训练就是在实际的算法工程师岗位上接受锻炼。如果这个比喻很恰当的话，那么一个模型的训练过程被分为多个阶段其实很好理解，因为不可能跳过上学直接工作，适当的预训练还是很重要的！！
...
另外，也可以考虑开辟一些新的语音方向，可以参考一些相关企业的岗位要求。很多岗位会写出一些不错的研究方向。
